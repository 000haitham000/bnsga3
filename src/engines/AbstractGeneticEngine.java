...
public abstract class AbstractGeneticEngine {
    ...
    protected double kktMetricLimit;
    protected double epsilon;
    protected int runIndex;
    protected ASF_Minimizer asfMinimizer;
    protected KKT_Calculator kktCalculator;
    // </editor-fold>
    ...
    protected boolean terminationCondition() {
        if (currentGenerationIndex == optimizationProblem.getGenerationsCount()
                || currentHV >= hvLimit
                || currentKktMetric <= kktMetricLimit
                || individualEvaluator.getFunctionEvaluationsCount() >= funcEvaluationsLimit) {
            return true;
        } else {
            return false;
        }
    ...
    public Individual[] generateInitialPopulation(String displayMessage) {
        ...
        if (DEBUG_ALL || DEBUG_POPULATIONS) {
            InputOutput.displayPopulation(optimizationProblem, displayMessage != null ? displayMessage : "", population);
        }
        return population;
    }
    ...
    ///*
    public void realCrossover_good(IndividualsSet parents, IndividualsSet children) {
        Individual parent1 = parents.getIndividual1();
        Individual parent2 = parents.getIndividual2();
        Individual child1 = children.getIndividual1();
        Individual child2 = children.getIndividual2();
    ...
    protected Individual[] merge(Individual[] oldPopulation, Individual[] newPopulation) {
        ...
        // references (the following two commented lines). Just to sure
        // practically sure about that we check for existing common references
        // (the line after) in the combined population, which will throw an 
        // exception if common references exist.
//        // Resolve common references in a population of individuals. 
//        combinedPopulation = resolveCommonReferences(combinedPopulation);
        checkCommonReferences(combinedPopulation, "STOP, SOMETHING IS WRONG");
        if (DEBUG_ALL || DEBUG_POPULATIONS) {
            InputOutput.displayPopulation(optimizationProblem, String.format("%d-population+%d-offspring", currentGenerationIndex, currentGenerationIndex), combinedPopulation);
        }
    ...
    public List<List<Individual>> rankIndividuals(Individual[] individuals, double epsilon, String optionalDisplayMessage) {
        int flag;
        int i;
        int end;
        int front_size;
        ...
        if (DEBUG_ALL || DEBUG_RANKING) {
            InputOutput.displayRanks(optionalDisplayMessage != null ? optionalDisplayMessage : "", individuals);
        }
        // Return the final list of fronts
        return fronts;
    }

    public double[] getInitialIdealPoint(Individual[] individuals, String optionalDisplayMessage) {
    ...
    protected void checkCommonReferencesList(List<Individual> individualsList, String message) {
    ...
    protected abstract void updateTerminationMetrics() throws UnsupportedOperationException, MWException;

    protected void displayPopulationIfDebug(int generationsCounter, Individual[] currentPopulation) {
        if (DEBUG_ALL || DEBUG_POPULATIONS) {
            System.out.format("--------------------------------------%n");
            System.out.format("Final Population After Generation(%d)%n", generationsCounter);
            System.out.format("--------------------------------------%n");
            InputOutput.displayPopulation(optimizationProblem, String.valueOf(generationsCounter + 1), currentPopulation);
        }
    }
    ...
    protected void updateObjectives(Individual[] individuals) {
        ...
        if (DEBUG_ALL || DEBUG_POPULATIONS) {
            InputOutput.displayPopulation(optimizationProblem, currentGenerationIndex + "-offspring", individuals);
        }
    }
...
//            String outputDir) throws IOException, MWException {
...
//                        + "/"
//                        + String.format("obj_gen_%04d.dat", generationsCounter);
//                InputOutput.collectObjectiveSpace(optimizationProblem, individualsToBePrinted, objSpaceFilePath);
//                // </editor-fold>
//
//                // <editor-fold desc="Dumping KKT metric values" defaultstate="collapsed">
//                // Dump the kktMetrics of the current generation
//                if (kktCalculator != null) {
//                    // Calculate min, median and max KKT errors
//                    // of the final population of this run.
//                    double[] kktErrors = kktCalculator.calculatePopulationKKT(individualsToBePrinted);
//                    String kktMetricFilePath = generationWiseDumpDir
//                            + "/"
//                            + String.format("gen_%04d_kkt.dat", generationsCounter);
//                    InputOutput.dumpDoubleArr(kktErrors, kktMetricFilePath);
//                }
//                // </editor-fold>
//
...
//                        + "/"
//                        + String.format("meta_data_gen_%04d.dat", generationsCounter);
//                InputOutput.dumpMetaData(metaDataFilePath, individualsToBePrinted, idealPoint, intercepts, evaluator.getFunctionEvaluationsCount(), utopianEpsilon);
//                // </editor-fold>
//
//                if (refDirsList != null) {
...
//                    // See the desired directiory structure at:
//                    // D:/corrected plotting script/version_2_11March2015
//                    String refDirsFilePath
//                            = generationWiseDumpDir
//                            + "/"
//                            + String.format("refdirs_gen_%04d.dat", generationsCounter);
//                    InputOutput.dumpRefDirs(refDirsFilePath, refDirsList);
//                    // </editor-fold>
//                }
//
//                // <editor-fold desc="Dumping promising reference directions if any exists" defaultstate="collapsed">
//                // Note that the notion of promising direction is not mandatory
//                // unless you are using some sort of smart niching. It does not
//                // come automatically with local search. Moreover, it may not
//                // be included in the final Matlab plot.
//                if (localSearchSummary != null && localSearchSummary.getPromisingDirectionsList() != null) {
//                    // Dump empty reference directions list
//                    String emptyRefDirsFilePath
//                            = generationWiseDumpDir
//                            + "/"
//                            + String.format("empty_refdirs_gen_%04d.dat", generationsCounter);
//                    InputOutput.dumpRefDirs(emptyRefDirsFilePath, localSearchSummary.getPromisingDirectionsList());
//                }
//                // </editor-fold>
//
//                // <editor-fold desc="Generate a normalized Matlab scripts for each generation" defaultstate="collapsed">
//                if (DUMP_ALL_GENERATIONS_NORMALIZED_MATLAB_SCRIPTS) {
//                    // Dump normalized Matlab plotting script of the current generation
//                    String normMatlabScriptFilePath
//                            = generationWiseDumpDir
//                            + "/"
//                            + String.format("gen_%04d_obj_matlab_normalized.m", generationsCounter);
//                    if (optimizationProblem.objectives.length == 2) {
//                        /*
//                         Make sure the resulting Matlab script conforms with the
//                         version at:
//                         D:\corrected plotting script\version_2_11March2015
//                         */
//                        InputOutput.dumpNormalizedMatlabPlottinScriptFor2dPoints(
//                                normMatlabScriptFilePath,
...
//                        InputOutput.dumpNormalizedMatlabPlottinScriptFor3dPoints(
//                                normMatlabScriptFilePath,
//                                objSpaceFilePath,
//                                individualsToBePrinted,
//                                idealPoint,
...
//                // </editor-fold>
//
//                // <editor-fold desc="Generate an animation script of the whole optimization process" defaultstate="collapsed">
//                // Make sure the animated script follows the rules of the latest
//                // modified version at:
//                // D:\corrected plotting script\version_2_11March2015
//                if (DUMP_ANIMATED_MATLAB_SCRIPT && generationsCounter == optimizationProblem.getGenerationsCount() - 1) {
//                    String animatedMatlabScriptFilePath
//                            = generationWiseDumpDir
//                            + "/"
//                            + String.format("matlab_animation.m", generationsCounter);
//                    if (optimizationProblem.objectives.length == 2) {
//                        InputOutput.dumpAnimatedMatlabScriptFor2dPoints(
//                                animatedMatlabScriptFilePath,
//                                ultimateIdealPoint,
//                                ultimateIntercepts,
//                                refDirsList);
//                    } else if (optimizationProblem.objectives.length == 3) {
//                        InputOutput.dumpAnimatedMatlabScriptFor3dPoints(
//                                animatedMatlabScriptFilePath,
//                                ultimateIdealPoint,
//                                ultimateIntercepts,
//                                refDirsList);
...
//                        = generationWiseDumpDir
//                        + "/"
//                        + String.format("matlab_gen_%04d_obj.m", generationsCounter);
//                // Dump the Matlab plotting script
//                if (optimizationProblem.objectives.length == 2) {
//                    InputOutput.createMatlabScript2D(matlabScriptFilePath, evaluator, OptimizationUtilities.getNonDominatedIndividuals(individualsToBePrinted, epsilon));
//                } else {
//                    InputOutput.createMatlabScript3D(matlabScriptFilePath, evaluator, OptimizationUtilities.getNonDominatedIndividuals(individualsToBePrinted, epsilon));
//                }
//            }
//            // </editor-fold>
//
...
//                        = generationWiseDumpDir
//                        + "/"
//                        + String.format("var_gen_%04d.dat", generationsCounter);
...
//    }
    ...
    protected abstract void fillUpPopulation(List<List<Individual>> fronts, Individual[] mergedPopulation) throws DoubleAssignmentException;
...
//        // Generate the matlab code that plots the set of non-dominated 
//        // individuals at the beginning of this iteration.
//        Individual[] feasibleSolutions = OptimizationUtilities.getFeasibleIndividuals(currentPopulation);
//        System.out.format("# Feasible Solutions: %03d%n", feasibleSolutions.length);
//
//        StringBuilder xSb = new StringBuilder("X = [");
//        StringBuilder ySb = new StringBuilder("Y = [");
//        for (int i = 0; i < feasibleSolutions.length; i++) {
//            xSb.append(feasibleSolutions[i].getObjective(0));
//            ySb.append(feasibleSolutions[i].getObjective(1));
//            if(i != feasibleSolutions.length - 1) {
//                xSb.append(", ");
//                ySb.append(", ");
//            }
//        }
...
//        codeBody.append(String.format("scatter(X,Y)%n"));
//        //codeBody.append(String.format("xlim([-260 30])%n"));
//        //codeBody.append(String.format("ylim([0 90])%n"));
//        System.out.println();
//        System.out.println(xSb.toString());
//        System.out.println(ySb.toString());
//        System.out.println(codeBody.toString());
//        System.out.println();
    }
    ...
    protected void report(File outputDir) throws MWException, IOException {
        HashMap<String, StringBuilder> dumpMap = reportGenerationWiseInfo();
        dumpMap.put("gen_count", new StringBuilder(String.valueOf(this.currentGenerationIndex)));
        InputOutput.dumpAll(outputDir, dumpMap);
    }

    public HashMap<String, StringBuilder> reportGenerationWiseInfo() throws IOException, MWException {
        ...
        if (DUMP_ALL_GENERATIONS_DECISION_SPACE || DUMP_ALL_GENERATIONS_OBJECTIVE_SPACE || DUMP_ALL_GENERATIONS_MATLAB_SCRIPTS || DUMP_ALL_GENERATIONS_NORMALIZED_MATLAB_SCRIPTS) {
            ...
            // Extract non-dominated solutions of the current generation
            Individual[] individualsToBePrinted
                    = OptimizationUtilities.getNonDominatedIndividuals(this.currentPopulation, epsilon);
            // </editor-fold>
            ...
            // <editor-fold desc="Dumping KKT metric values" defaultstate="collapsed">
            // Dump the kktMetrics of the current generation
            if (DUMP_ALL_GENERATIONS_KKTPM) {
                if (kktCalculator != null) {
                    StringBuilder kktpmSb = InputOutput.collectKKTPM(
                            kktCalculator,
                            individualsToBePrinted,
                            OptimizationUtilities.pullPointBack(
                                    individualEvaluator.getIdealPoint(), 0.01));
            ...
            if (DUMP_ALL_GENERATIONS_META_DATA) {
                StringBuilder metaDataSb = InputOutput.collectMetaData(
                        ...
                        this.individualEvaluator.getNumericalFunctionEvalCount(), 
                        epsilon);
            ...
            if (DUMP_ALL_GENERATIONS_MATLAB_SCRIPTS && optimizationProblem.objectives.length <= 3) {
                ...
                if (optimizationProblem.objectives.length == 2) {
                    sb = InputOutput.createMatlabScript2D(OptimizationUtilities.getNonDominatedIndividuals(individualsToBePrinted, epsilon));
                } else if (optimizationProblem.objectives.length == 3) {
                    sb = InputOutput.createMatlabScript3D(OptimizationUtilities.getNonDominatedIndividuals(individualsToBePrinted, epsilon));
                }
                if (sb != null) {
            ...
            if (DUMP_ALL_GENERATIONS_DECISION_SPACE) {
                // Dump Decision Space
                StringBuilder desSpace = InputOutput.collectRealDecisionSpace(optimizationProblem, individualsToBePrinted);
