...
public class WeldedBeamEvaluator extends IndividualEvaluator {
    ...
    public void updateIndividualObjectivesAndConstraints(
        ...
        double td = 13600.0, sd = 30000.0, gmod = 12.0e06, emod = 30.0e06, force = 6000.0;

        double[] x = individual.real;
        double obj0 = (1.0 + 0.37 * 0.283) * x[0] * x[0] * x[1] + 0.17 * 0.283 * x[2] * x[3] * (14.0 + x[1]);
        double obj1 = 4.0 * force * 14.0 * 14.0 * 14.0 / (emod * x[2] * x[2] * x[2] * x[3]);
        ...
        if (problem.constraints != null) {
            ...
            double rad = Math.sqrt(x[1] * x[1] / 4.0 + Math.pow(0.5 * (x[2] + x[0]),2));
            double jmod = 2.0 * (x[0] * x[1] * (x[1] * x[1] / 12.0 + Math.pow(0.5 * (x[0] + x[2]),2)) / Math.sqrt(2.0));

            double t1 = force / (Math.sqrt(2.0) * x[0] * x[1]);
            double t2 = mom * rad / jmod;
            double ctheta = x[1] / (2.0 * rad);
            double tx = Math.sqrt(Math.pow(t1,2) + Math.pow(t2,2) + 2.0 * t1 * t2 * ctheta);
            double sx = 6.0 * force * 14.0 / (x[3] * x[2] * x[2]);
            double ei = emod * x[2] * x[3] * x[3] * x[3] / 12.0;
            double alpha = gmod * x[2] * x[3] * x[3] * x[3] / 3.0;
            double pc = 4.013 * Math.sqrt(ei * alpha) * (1.0 - x[2] * Math.sqrt(ei / alpha) / (2.0 * 14.0))
                    / (14.0 * 14.0);
            // Four constraints
