...
public class SingleObjective_G07_Evaluator extends IndividualEvaluator {
    ...
    public void updateIndividualObjectivesAndConstraints(
        ...
        if (problem.constraints != null) {
            ...
            g[3] = 3 * Math.pow(x[0] - 2, 2) + 4 * Math.pow(x[1] - 3, 2) + 2 * Math.pow(x[2], 2) - 7 * x[3] - 120;
            g[4] = 5 * Math.pow(x[0], 2) + 8 * x[1] + Math.pow(x[2] - 6, 2) - 2 * x[3] - 40;
            g[5] = Math.pow(x[0], 2) + 2 * Math.pow(x[1] - 2, 2) - 2 * x[0] * x[1] + 14 * x[4] - 6 * x[5];
            g[6] = 0.5 * Math.pow(x[0] - 8, 2) + 2 * Math.pow(x[1] - 4, 2) + 3 * Math.pow(x[4], 2) - x[5] - 30;
