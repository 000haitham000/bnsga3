...
public class SingleObjective_G09_Evaluator extends IndividualEvaluator {
    ...
    public void updateIndividualObjectivesAndConstraints(
        ...
        if (problem.constraints != null) {
            ...
            g[0] = (-127 + 2 * Math.pow(x[0], 2) + 3 * Math.pow(x[1], 4) + x[2] + 4 * Math.pow(x[3], 2) + 5 * x[4]) / 127;
            g[1] = (-282 + 7 * x[0] + 3 * x[1] + 10 * Math.pow(x[2], 2) + x[3] - x[4]) / 282;
            g[2] = (-196 + 23 * x[0] + Math.pow(x[1], 2) + 6 * Math.pow(x[5], 2) - 8 * x[6]) / 196;
            g[3] = 4 * Math.pow(x[0], 2) + Math.pow(x[1], 2) - 3 * x[0] * x[1] + 2 * Math.pow(x[2], 2) + 5 * x[5] - 11 * x[6];
            // Reverse constraints (the original paper(technical report) uses <= notation)
