...
public class GeneralC3DTLZ4Evaluator extends IndividualEvaluator {
    ...
    public void updateIndividualObjectivesAndConstraints(
        ...
        if (problem.constraints != null) {
            // In C3-DTLZ4, the number of constraints is equal to the number of objectives
            double[] constraints = new double[problem.objectives.length];
            for (int j = 0; j < constraints.length; j++) {
                constraints[j] = Math.pow(individual.getObjective(j), 2)/4 - 1;
                for (int i = 0; i < problem.objectives.length; i++) {
                    if (i != j) {
                        constraints[j] += Math.pow(individual.getObjective(i), 2);
                    }
                }
            }
