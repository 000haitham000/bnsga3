...
public class ShipModelEvaluator extends IndividualEvaluator {
    ...
    public void updateIndividualObjectivesAndConstraints(
        ...
        double MassSteel = 0.034 * Math.pow(L, 1.7) * Math.pow(B, 0.7) * Math.pow(D, 0.4) * Math.pow(Cb, 0.5);
        double MassOutfit = 1.0 * Math.pow(L, 0.8) * Math.pow(B, 0.6) * Math.pow(D, 0.3) * Math.pow(Cb, 0.1);
        ...
        double P = Math.pow(displacement, (2.0 / 3.0)) * Math.pow(V, 3.0) / (b_Cb * Fn + a_Cb);
        double GM = KB + BM - KG;

        double ShipCost = 1.3 * (2000.0 * Math.pow(MassSteel, 0.85) + 3500.0 * MassOutfit + 2400.0 * Math.pow(P, 0.8));
