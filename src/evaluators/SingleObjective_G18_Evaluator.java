...
public class SingleObjective_G18_Evaluator extends IndividualEvaluator {
    ...
    public void updateIndividualObjectivesAndConstraints(
        ...
        double obj0 = -0.5 * (x[0] * x[3] - x[1] * x[2] + x[2] * x[8] - x[4] * x[8] + x[4] * x[7] - x[5] * x[6]);
        ...
        if (problem.constraints != null) {
            ...
            g[9] = x[1] * x[2] - x[0] * x[3]; // Corrected by Mohamed on 27th Oct. 2014 (before I was mistaken when I wrote it like this: x[1] * x[2] - x[0] * x[4])
            g[10] = -1 * x[2] * x[8];
            g[11] = x[4] * x[8];
