 ...
 * @author Haitham
 */
public class Zdt6KKTPMCalculatorNumerical extends KKTPM_CalculatorImpl {

    public Zdt6KKTPMCalculatorNumerical() {
        ...
        if (problem == null) {
            try {
                problem = XMLParser.readXML(new File("E:/KKTPM/Java KKTPM/XML/zdt6_no_gradients.xml"));
                problem.setConstant("pi", Math.PI);
                problem.setConstant("e", Math.E);
            } catch (Throwable ex) {
                Logger.getLogger(Zdt6KKTPMCalculatorNumerical.class.getName()).log(Level.SEVERE, null, ex);
