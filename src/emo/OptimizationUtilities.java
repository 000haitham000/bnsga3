...
public class OptimizationUtilities {
    ...
    public static Individual[] getFeasibleIndividuals(Individual[] individuals) {
        ...
        Individual[] feasibleIndividualsArr = new Individual[feasibleIndividuals.size()];
    ...
    public static double getMinObjectiveValue(Individual[] individuals, int objectiveIndex) {
    ...
    public static double getMaxObjectiveValue(Individual[] individuals, int objectiveIndex) {
    ...
    public static Individual[] getNonDominatedIndividuals(Individual[] individuals, double epsilon) {
    ...
    public static VirtualIndividual[] getNonDominatedIndividuals(VirtualIndividual[] individuals, double epsilon) {
        List<VirtualIndividual> nonDominatedList = new ArrayList<VirtualIndividual>();
        outerLoop:
        for (int ii = 0; ii < individuals.length; ii++) {
            for (int jj = 0; jj < individuals.length; jj++) {
                if (jj != ii) {
                    ...
                    for (int i = 0; i < individuals[ii].getObjectivesCount(); i++) {
                        if (Mathematics.compare(individuals[ii].getObjective(i) * (1 - epsilon), individuals[jj].getObjective(i)) == -1) {
                            indv1IsBetterInSomeObjective = 1;
                        } else {
                            if (Mathematics.compare(individuals[ii].getObjective(i), individuals[jj].getObjective(i) * (1 - epsilon)) == 1) {
                                individual2IsBetterInSomeObjective = 1;
                            }
                        }
                    }

                    if (indv1IsBetterInSomeObjective == 0 && individual2IsBetterInSomeObjective == 1) {
                        continue outerLoop;
                    }
        ...
        VirtualIndividual[] nonDominatedArr = new VirtualIndividual[nonDominatedList.size()];
        nonDominatedList.toArray(nonDominatedArr);
        return nonDominatedArr;
    }
    ...
    public static Individual[] getCandidates(
        ...
        // ===============================================
        //for (int h = 0; h < individuals.length; h++) {
        //    System.out.println(String.format("IND(%d): %d%n", h, individuals[h].getRank()));
        //}
        //InputOutput.displayFronts(fronts);
        // ===============================================
        ...
        //int count = 0;
        //for (int h = 0; h < individuals.length; h++) {
        //    if (individuals[h].getRank() < frontOrder) {
        //        System.out.println(h + ": " + individuals[h].getRank() + "(" + (count++) + ")");
        //    }
        //}
        // ===============================================
        int i = 0;
...
//            // have been found since their previous translation (in the
//            // previous generation).
//            for (int i = 0; i < idealPoint.length; i++) {
//                for (int j = 0; j < idealPoint.length; j++) {
...
//            }
            ...
            for (int i = 0; i < previousExtremePoints.length; i++) {
                ...
                previousMaxValues[i] = (previousExtremePoints[i].getObjective(0) - idealPoint[0]) / wDirection[0];
                for (int k = 1; k < idealPoint.length; k++) {
                    double nextValue = (previousExtremePoints[i].getObjective(k) - idealPoint[k]) / wDirection[k];
        ...
        VirtualIndividual[] extremePoints = new VirtualIndividual[idealPoint.length];
        ...
        for (int i = 0; i < unitDirections.length; i++) {
            ...
            for (int j = 0; j < individuals.length; j++) {
                if (EXTREME_POINTS_DEEP_DEBUG) {
                    ...
                    System.out.format("= max(%20.2f, %20.2f, %20.2f)%n",
                            (individuals[j].getObjective(0) - idealPoint[0]) / wDirection[0],
                            (individuals[j].getObjective(1) - idealPoint[1]) / wDirection[1],
                            (individuals[j].getObjective(2) - idealPoint[2]) / wDirection[2]);
                }
                double max = (individuals[j].getObjective(0) - idealPoint[0]) / wDirection[0];
                for (int k = 1; k < idealPoint.length; k++) {
                    double nextValue = (individuals[j].getObjective(k) - idealPoint[k]) / wDirection[k];
                    if (nextValue > max) {
                        max = nextValue;
                    }
                }
            ...
            if (previousExtremePoints != null && previousMaxValues[i] < maxArr[minIndex]) {
                ...
                if (individuals[minIndex] instanceof Individual) {
                    extremePoints[i] = new Individual((Individual) individuals[minIndex]);
                } else {
                    extremePoints[i] = new VirtualIndividual(individuals[minIndex]);
                }
                // Translate extreme point
//                for (int j = 0; j < idealPoint.length; j++) {
//                    extremePoints[i].setObjective(j, extremePoints[i].getObjective(j) - idealPoint[j]);
//                }
                if (EXTREME_POINTS_DEEP_DEBUG) {
...
//                }
    ...
    public static Individual[] selectOnlyOneIndividualForEachDirection(Individual[] individuals, List<ReferenceDirection> referenceDirectionsList) {
        ...
        // Create an empty list to which directions with no attached individuals will be kept
        List<ReferenceDirection> isolatedDirsList = new ArrayList<ReferenceDirection>();
        ...
        for (ReferenceDirection dir : referenceDirectionsList) {
            ...
            for (i = 0; i < feasibleIndividualsList.size(); i++) {
                if (feasibleIndividualsList.get(i).getReferenceDirection().equals(dir)) {
            ...
            if (minDistIndividual == null) {
                ...
                for (; i < feasibleIndividualsList.size(); i++) {
                    if (feasibleIndividualsList.get(i).getReferenceDirection().equals(dir)
                            && feasibleIndividualsList.get(i).getPerpendicularDistance() < minDistIndividual.getPerpendicularDistance()) {
                        minDistIndividual = feasibleIndividualsList.get(i);
                    }
                }
        ...
        for (int i = 0; i < referenceDirectionsList.size() && !feasibleIndividualsList.isEmpty(); i++) {
            ...
            for (int j = 1; j < feasibleIndividualsList.size(); j++) {
                if (feasibleIndividualsList.get(j).distancesFromDirs[i] < minDistIndividual.distancesFromDirs[i]) {
        ...
        Individual[] selectedIndividualsArr = new Individual[selectedIndividuals.size()];
    ...
    public static void fixVariableLimits(OptimizationProblem optimizationProblem, double[] x) {
        List<OptimizationProblem.RealVariableSpecs> specs
                = new ArrayList<OptimizationProblem.RealVariableSpecs>();
        for (OptimizationProblem.Variable variableSpecs : optimizationProblem.variablesSpecs) {
            if (variableSpecs instanceof OptimizationProblem.RealVariableSpecs) {
                specs.add((OptimizationProblem.RealVariableSpecs) variableSpecs);
            }
        }
    ...
    public static void main(String[] args) throws IOException {
        VirtualIndividual[] individuals = InputOutput.loadIndividualsFromFile(new File("d:/temp/pop.txt"));
        VirtualIndividual[] nonDominated = OptimizationUtilities.getNonDominatedIndividuals(individuals, 0);
        for (VirtualIndividual individual : nonDominated) {
            for (int i = 0; i < individual.getObjectivesCount(); i++) {
                System.out.format("%-7.5f ", individual.getObjective(i));
            }
            System.out.println();
        }
    }
