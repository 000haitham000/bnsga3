...
public class Individual extends VirtualIndividual implements Comparable<Individual> {
    ...
    public boolean validConstraintsViolationValues, validRankValue, validReferenceDirection;
    private boolean nsga2crowdingDistanceAlreadySet = false;
    private double nsga2crowdingDistance;

    public Individual(OptimizationProblem problem, IndividualEvaluator individualEvaluator) {
        this(problem, individualEvaluator, null);
    }

    public Individual(OptimizationProblem problem, IndividualEvaluator individualEvaluator, double[] realDesignVariables) {
        ...
        for (int i = 0; i < problem.variablesSpecs.length; i++) {
            if (problem.variablesSpecs[i] instanceof OptimizationProblem.BinaryVariableSpecs) {
                ...
                if (realDesignVariables == null) {
                    ...
                    double rndreal = RandomNumberGenerator.rndreal(problem.variablesSpecs[i].getMinValue(), problem.variablesSpecs[i].getMaxValue());
        ...
        //this.real[1] = 0.5;
        //this.real[2] = 0.7;
        //for (int j = 3; j < this.real.length; j++) {
        //    this.real[j] = 0;
        //}
        //*****************************************************
    ...
    public boolean isFeasible() {
        if (Mathematics.compare(this.getTotalConstraintViolation(), 0, Mathematics.EPSILON) == 0) {
            return true;
        } else {
            return false;
        }
    ...
    public String getObjectiveSpace() {
        ...
        result += String.format(" - CV = %05.2f%n", this.getTotalConstraintViolation());
        return result;
    }
    ...
    public String toString() {
        ...
        if (validObjectiveFunctionsValues) {
            for (int i = 0; i < this.objectiveFunction.length; i++) {
                result += String.format(" {Obj(%d) = %-15.10f}", i + 1, objectiveFunction[i]);
            }
        } else {
        ...
        if (validConstraintsViolationValues && this.constraintViolation != null) {
            for (int i = 0; i < this.constraintViolation.length; i++) {
                result += String.format(" {Constraint(%d) = %-15.10f}", i + 1, constraintViolation[i]);
            }
            // Total Constraints Violation
            result += String.format(" (Total Violations: %-15.10f)", getTotalConstraintViolation());
        } else {
            result += "Invalid (outdated)";
        }
...
//    }
    ...
    public ReferenceDirection getReferenceDirection() {
        if (validReferenceDirection) {
            return new ReferenceDirection(
                    Arrays.copyOf(this.referenceDirection.direction, this.referenceDirection.direction.length));
        } else {
    ...
    public void setReferenceDirection(ReferenceDirection referenceDirection) {
        if (referenceDirection == null) {
            ...
            this.referenceDirection = new ReferenceDirection(
                    Arrays.copyOf(referenceDirection.direction, referenceDirection.direction.length));
        }
    }
    ...
    public int compareTo(Individual individual) {
        if (nsga2comparisonObjectiveIndex == -1) {
            if (Mathematics.compare(this.nsga2crowdingDistance, AbstractGeneticEngine.MAX_DOUBLE_VALUE) == 0
                    && Mathematics.compare(individual.nsga2crowdingDistance, AbstractGeneticEngine.MAX_DOUBLE_VALUE) == 0) {
                return 0;
            }
            double crowdedDistDiff = this.nsga2crowdingDistance - individual.nsga2crowdingDistance;
        ...
        if (this.isFeasible()) {
            if (individual.isFeasible()) {
                /*
                 ...
                 return false;
                 } else if (Mathematics.compare(this.objectiveFunction[i], individual.objectiveFunction[i]) == -1) {
                 ...
                 */
                // Both are non-violating (feasible) (the normal NSGA-II dominance principle is applied here)
                int indv1IsBetterInSomeObjective = 0;
                int individual2IsBetterInSomeObjective = 0;
                for (int i = 0; i < optimizationProblem.objectives.length; i++) {
                    if (Mathematics.compare(this.getObjective(i) * (1 - epsilon), individual.getObjective(i)) == -1) {
                        indv1IsBetterInSomeObjective = 1;
                    } else {
                        if (Mathematics.compare(this.getObjective(i), individual.getObjective(i) * (1 - epsilon)) == 1) {
                            individual2IsBetterInSomeObjective = 1;
                        }
                    }
                }
                if (indv1IsBetterInSomeObjective == 1 && individual2IsBetterInSomeObjective == 0) {
                    return true;
                } else {
            ...
            if (individual.isFeasible()) {
                ...
                if (Mathematics.compare(this.getTotalConstraintViolation(), individual.getTotalConstraintViolation()) == 1) {
                    return true;
                } else if (Mathematics.compare(this.getTotalConstraintViolation(), individual.getTotalConstraintViolation()) == -1) {
                    return false;
                } else {
     ...
     */
    public void setNsga2crowdingDistance(double nsga2crowdingDistance) throws DoubleAssignmentException {
    ...
    public static void restartCrowdingDistanceCalculations(Individual[] individuals) throws DoubleAssignmentException {
    ...
    public static void restartProposedCrowdingDistanceMeasures(Individual[] individuals) {
        for (Individual individual : individuals) {
