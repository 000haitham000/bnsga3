...
public class NestedReferenceDirectionsFactory extends ReferenceDirectionsFactory {
    ...
    public List generateDirections(int[] divisions) {
        ...
        List<ReferenceDirection> directions = new ArrayList<ReferenceDirection>();
        ...
        for (int i = 0; i < divisions.length; i++) {
            ...
            ReferenceDirectionsFactory directionsGenerator = new ReferenceDirectionsFactory(objectivesCount, sum);
            // Generate the directions (un-augmented)
            List<ReferenceDirection> levelDirections = directionsGenerator.generateDirections(divisions[i]);
            ...
            double[] levelAugmentationVector = getAugmentationVector(objectivesCount, (1.0 - sum) / objectivesCount);
        ...
        double[] centralVector = getAugmentationVector(objectivesCount, 1.0/objectivesCount);
    ...
    protected void augment(ReferenceDirection dir, double[] augmentationVector) {
    ...
    private static void check(ReferenceDirection dir, int objectivesCount) {
        ...
        if (Mathematics.compare(sum, 1, 0.000001) != 0) {
            ...
            throw new IllegalArgumentException("Coordinates do not sum up to one.");
        }
    }
        ...
        for (int i = 0; i < dirs.size() - 1; i++) {
            for (int j = i+1; j < dirs.size(); j++) {
                if(dirs.get(i).equals(dirs.get(j))) {
                    repetitionCount++;
                    repetitionReport.append(String.format("Directions (%d) & (%d) are repeated: %s%n", i, j, dirs.get(i).toString()));
                }
            }
        }
        if(repetitionCount != 0) {
            repetitionReport.append(String.format("%n# repetitions = %d%n", repetitionCount));
    ...
    public static void main(String[] args) {
        ...
        NestedReferenceDirectionsFactory nestedReferenceDirectionsFactory = new NestedReferenceDirectionsFactory(objCount);
        List<ReferenceDirection> dirs = nestedReferenceDirectionsFactory.generateDirections(divisionsCountArr);
        ...
        for (ReferenceDirection dir : dirs) {
            ...
            try {
                ...
                System.out.println(" - FAILED (coordinates do not sum up to one)");
            }
            count++;
        }
        System.out.format("%n%d passed the validation out of %d%n", passCount, dirs.size());
        System.out.format("# directions = %d%n%n", dirs.size());
        try {
        ...
        System.out.println();
        InputOutput.printDirectionMatlabCode(objCount, dirs, new PrintWriter(System.out));
    }
}
